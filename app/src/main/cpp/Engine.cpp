        ...
        acceleration{}
{
    ...
    sensorEventQueue = ASensorManager_createEventQueue(sensorManager, pApp->looper,
            LOOPER_ID_USER, nullptr, &sensorPollSource);
...
void Engine::handleInput() {
    ...
    std::vector<TouchEvent> touchEvents;
    {
        ...
        touchEvents.swap(touchEvents_);
    }

    for (const auto& event : touchEvents) {
        // Process the touch event here
        aout << "Touch Event: action=" << event.action << ", x=" << event.x << ", y=" << event.y << std::endl;
    }
}

void Engine::pushTouchEvent(int32_t action, float x, float y) {
    std::lock_guard<std::mutex> lock(inputMutex_);
    touchEvents_.push_back({action, x, y});
}
